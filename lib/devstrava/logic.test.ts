...
// @ts-expect-error Node's strip-types test runner resolves TypeScript extensions directly.
import { parseDevStravaImportText } from "./contract.ts";
...
test("accepts the v1 contract and skips an existing duplicate", () => {
  ...
  const result = parseDevStravaImportText(payload, new Set([DEMO_SESSIONS[0].sessionId]));
...
test("rejects unsupported future schema versions", () => {
  const result = parseDevStravaImportText(JSON.stringify({ schemaVersion: 2, sessions: [] }));
...
test("rejects malformed percentages and timestamp ordering", () => {
  ...
  const result = parseDevStravaImportText(JSON.stringify(malformed));
  assert.equal(result.ok, false);
  assert.equal(result.sessions.length, 0);
