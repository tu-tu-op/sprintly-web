...
function SidebarContent({ mobile = false }: { mobile?: boolean }) {
  ...
  const { displayName, initials, syncLabel, sessionsCount } = useShellState();
  ...
  return <div className="flex h-full flex-col p-3">
    <div className={`flex min-h-12 items-center ${isCollapsed ? "justify-center" : "justify-between px-1"}`}>
      {isCollapsed ? <Link prefetch={false} href="/app" aria-label="Sprintly overview"><SprintlyMark className="size-8"/></Link> : <Brand />}
    ...
    <nav className="mt-5 space-y-1" aria-label="Product navigation">{nav.map(({label,href,icon:Icon})=>{const active=href==="/app"?pathname===href:pathname.startsWith(href);const prefetch=()=>router.prefetch(href);return <Link prefetch={false} key={href} href={href} onMouseEnter={prefetch} onFocus={prefetch} onClick={()=>mobile&&setMobileOpen(false)} title={isCollapsed?label:undefined} aria-current={active?"page":undefined} className={`group flex min-h-11 items-center rounded-lg text-sm transition ${isCollapsed?"justify-center px-0":"gap-3 px-3"} ${active?"bg-[#f2f2f2]/12 text-[#e6e6e6]":"text-[#8b8b8b] hover:bg-white/[.045] hover:text-[#dcdcdc]"}`}><Icon aria-hidden="true" className={`size-[18px] shrink-0 ${active?"text-[#d6d6d6]":"text-[#737373] group-hover:text-[#a1a1a1]"}`}/>{!isCollapsed&&<span>{label}</span>}{active&&!isCollapsed&&<span className="ml-auto size-1.5 rounded-full bg-[#f2f2f2]"/>}</Link>})}</nav>
    <div className="mt-auto space-y-2">
      <div className={`rounded-xl border border-[#d0d0d0]/15 bg-[#d0d0d0]/[.045] ${isCollapsed?"grid min-h-11 place-items-center":"p-3"}`} title={isCollapsed?syncLabel:undefined}>{isCollapsed?<CheckCircle2 className="size-4 text-[#d0d0d0]"/>:<><div className="flex items-center gap-2 text-xs font-medium text-[#c3c3c3]"><CheckCircle2 className="size-4"/> {syncLabel}</div><p className="mt-2 text-[11px] leading-5 text-[#7b7b7b]">{sessionsCount} session{sessionsCount === 1 ? "" : "s"} stored locally</p></>}</div>
      <Link prefetch={false} href="/app/profile" onMouseEnter={()=>router.prefetch("/app/profile")} onFocus={()=>router.prefetch("/app/profile")} className={`flex min-h-12 items-center rounded-xl border border-white/[.07] bg-white/[.025] ${isCollapsed?"justify-center":"gap-3 p-2"}`}><div className="grid size-8 shrink-0 place-items-center rounded-lg bg-gradient-to-br from-[#f2f2f2] to-[#bdbdbd] text-xs font-semibold">{initials}</div>{!isCollapsed&&<><div className="min-w-0"><p className="truncate text-xs font-medium">{displayName}</p><p className="truncate text-[10px] text-[#797979]">Local demo account</p></div><ChevronRight className="ml-auto size-4 text-[#6a6a6a]"/></>}</Link>
    </div>
  </div>;
}
...
const NOTIFICATION_ICONS = { streak: Flame, achievement: Trophy, storage: CheckCircle2 } as const;

function Notifications(){const { notifications, sessionsCount } = useShellState();return <DropdownMenu.Root><DropdownMenu.Trigger asChild><button aria-label="Open notifications" className="grid size-11 place-items-center rounded-lg border border-white/[.07] bg-white/[.025] text-[#989898] transition hover:bg-white/[.06] hover:text-white"><Bell className="size-[18px]"/></button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content align="end" sideOffset={8} className="z-[80] w-[min(360px,calc(100vw-24px))] rounded-xl border border-white/10 bg-[#151515] p-2 shadow-2xl"><div className="flex items-center justify-between px-3 py-2"><p className="text-sm font-semibold">Notifications</p><span className="text-[10px] text-[#f2f2f2]">{sessionsCount} stored</span></div>{notifications.map((item)=>{const Icon=NOTIFICATION_ICONS[item.kind];return <DropdownMenu.Item key={item.id} className="flex gap-3 rounded-lg p-3 outline-none transition hover:bg-white/[.05]"><div className="grid size-9 shrink-0 place-items-center rounded-lg bg-[#f2f2f2]/10 text-[#d2d2d2]"><Icon className="size-4"/></div><div><p className="text-xs font-medium text-[#dbdbdb]">{item.title}</p><p className="mt-1 text-[11px] text-[#7e7e7e]">{item.description}</p></div></DropdownMenu.Item>})}</DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>}
