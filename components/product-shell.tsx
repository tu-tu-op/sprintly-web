...
import {
  ...
  Flame, LayoutDashboard, Menu, PanelLeftClose, PanelLeftOpen,
...
function SidebarContent({ mobile = false }: { mobile?: boolean }) {
  ...
  return <div className="flex h-full flex-col p-3">
    ...
    <div className="mt-auto space-y-2">
      ...
      <Link href="/app/profile" onMouseEnter={()=>router.prefetch("/app/profile")} onFocus={()=>router.prefetch("/app/profile")} className={`flex min-h-12 items-center rounded-xl border border-white/[.07] bg-white/[.025] ${isCollapsed?"justify-center":"gap-3 p-2"}`}><div className="grid size-8 shrink-0 place-items-center rounded-lg bg-gradient-to-br from-[#f2f2f2] to-[#bdbdbd] text-xs font-semibold">{initials}</div>{!isCollapsed&&<><div className="min-w-0"><p className="truncate text-xs font-medium">{displayName}</p><p className="truncate text-[10px] text-[#797979]">Local demo account</p></div><ChevronRight className="ml-auto size-4 text-[#6a6a6a]"/></>}</Link>
    </div>
  </div>;
}
