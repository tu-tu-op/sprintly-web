...
function number(value: number) { return numberFormatter.format(Math.round(value)); }
...
function dateLabel(value: string) { return dateFormatter.format(new Date(value)); }
...
export function SessionsPage() {
  ...
  const records = useMemo(() => recordsOf(sessions), [sessions]);
  // Refresh calendar-relative results on the next interaction after midnight.
  const today = dateKey(new Date(), preferences.timeZone);
  const visible = useMemo(() => filterSessionsByRange(records, range, new Date(), custom, preferences.timeZone).sort((a, b) => sort === "longest" ? b.activeDurationSeconds - a.activeDurationSeconds : sort === "score" ? sessionCompositeScore(b) - sessionCompositeScore(a) : b.startedAt.localeCompare(a.startedAt)), [records, range, sort, custom, preferences.timeZone, today]);
  const aggregate = useMemo(() => aggregateSessions(visible), [visible]);
  const pageKey = [range, sort, custom.from, custom.to, preferences.timeZone, today].join("|");
