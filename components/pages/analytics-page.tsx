...
import {
  ...
  computePersonalRecords, dateKey, getIsoWeek, filterSessionsByRange, formatDuration, getPreviousPeriodBounds, getStreakStats,
...
export function AnalyticsPage() {
  ...
  const weeklyAggregate = aggregateSessions(filterSessionsByRange(records, "week", new Date(), undefined, timeZone));
  ...
  return <div>
    ...
    <div className="mt-3 grid gap-3 md:grid-cols-2"><Panel className="p-5"><div className="flex items-center justify-between"><div><h2 className="text-sm font-semibold">Weekly recap</h2><p className="mt-1 text-xs text-[#7a7a7a]">Your current week, ready to share</p></div><Link href={{ pathname: "/app/share", query: { kind: "weekly", sourceId: "weekly:" + getIsoWeek(new Date(), timeZone) } }} className="inline-flex min-h-9 items-center gap-2 rounded-lg bg-[#f2f2f2] text-[#0b0b0b] px-3 text-xs font-semibold">Share this week <Upload className="size-3.5" /></Link></div><p className="mt-6 text-2xl font-semibold tracking-[-.04em]">{weeklyAggregate.sessionCount} sessions · {formatDuration(weeklyAggregate.activeDurationSeconds)}</p><p className="mt-2 text-xs text-[#8b8b8b]">{weeklyAggregate.coding.aiAssistedPercent}% AI-assisted · {weeklyAggregate.reliability.recoveryRate}% recovery · {streaks.current} day streak.</p></Panel><Panel className="p-5"><h2 className="text-sm font-semibold">Privacy boundary</h2><p className="mt-3 text-xs leading-5 text-[#858585]">These aggregates are computed from selected sessions. Raw prompts, code, terminal output, and secrets never enter the social layer.</p><Link href="/app/settings" className="mt-5 inline-flex min-h-10 items-center gap-2 text-xs font-semibold text-[#c2c2c2]">Review privacy controls <ChevronRight className="size-3.5" /></Link></Panel></div>
  </div>;
}
