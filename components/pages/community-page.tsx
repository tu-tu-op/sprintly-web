...
export function CommunityPage() {
  ...
  const entries = repositoryMode === "remote"
    ? remoteEntries.map((entry) => ({ id: entry.user_id, name: entry.display_name, handle: `@${entry.handle.replace(/^@/, "")}`, avatar: initials(entry.display_name), country: "", city: "", friends: [], schemaVersion: 1 as const, devScoreVersion: "v1" as const, week: "current", region: scopeKey, sessions: entry.session_count, activeMinutes: Math.round(entry.focus_time_seconds / 60), focusScore: Math.round(entry.consistency_score), recoveryScore: 0, devScore: Math.round(entry.score), streak: 0 }))
    : localEntry;
