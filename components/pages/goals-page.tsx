...
export function GoalsPage(){return <div><PageHeader eyebrow="Goals & sprints" title="Turn intention into momentum." description="Set goals around consistency and outcomes—not passive screen time." action={<button onClick={()=>toast.success("Goal draft opened")} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-[#f2f2f2] text-[#0b0b0b] px-4 text-sm font-semibold"><Plus className="size-4"/> Create goal</button>}/><Panel className="mb-3 overflow-hidden p-5 sm:p-7"><div className="grid gap-8 lg:grid-cols-[1fr_1.1fr] lg:items-center"><div><Tag color="violet"><Zap className="size-3"/> Sprint 08 · strong momentum</Tag><h2 className="mt-5 text-3xl font-semibold tracking-[-.05em]">Ship activity timeline</h2><p className="mt-3 text-sm leading-6 text-[#8b8b8b]">A 10-task product sprint connected to 8 focused sessions and 11h 42m of work.</p><div className="mt-7 flex gap-8"><div><p className="mono text-2xl font-semibold">68%</p><p className="mt-1 text-xs text-[#7a7a7a]">progress</p></div><div><p className="mono text-2xl font-semibold">2d</p><p className="mt-1 text-xs text-[#7a7a7a]">remaining</p></div><div><p className="mono text-2xl font-semibold">7/10</p><p className="mt-1 text-xs text-[#7a7a7a]">tasks</p></div></div></div><div><div className="relative h-3 overflow-hidden rounded-full bg-white/[.06]"><motion.div initial={{width:0}} animate={{width:"68%"}} className="h-full rounded-full bg-gradient-to-r from-[#f2f2f2] to-[#bdbdbd]"/></div><div className="mt-4 grid grid-cols-5 gap-2">{[1,2,3,4,5].map((m,i)=><div key={m} className={`rounded-lg border p-3 text-center ${i<3?"border-[#f2f2f2]/20 bg-[#f2f2f2]/8":"border-white/[.07] bg-white/[.02]"}`}><CheckCircle2 className={`mx-auto size-4 ${i<3?"text-[#d2d2d2]":"text-[#5b5b5b]"}`}/><p className="mono mt-2 text-[9px] text-[#787878]">M{m}</p></div>)}</div><div className="mt-4 flex justify-between text-xs text-[#7d7d7d]"><span>Foundation</span><span>Polish & ship</span></div></div></div></Panel><div className="grid gap-3 md:grid-cols-2">{goals.map(g=><Panel key={g.title} className="p-5"><div className="flex justify-between"><Tag color={g.type==='Daily'?"cyan":g.type==='Weekly'?"green":g.type==='Sprint'?"violet":"amber"}>{g.type}</Tag><button aria-label={`Options for ${g.title}`} className="grid size-9 place-items-center rounded-lg text-[#6e6e6e]"><MoreHorizontal className="size-4"/></button></div><h2 className="mt-6 text-lg font-semibold tracking-[-.025em]">{g.title}</h2><p className="mt-2 text-xs text-[#838383]">{g.detail}</p><div className="mt-6"><Progress value={g.value} color={g.color}/></div><div className="mt-4 flex justify-between text-[11px] text-[#797979]"><span>{g.value}% complete</span><span>{g.due}</span></div></Panel>)}</div></div>}
