...
const goals=[{title:"20 focused hours",type:"Weekly",value:92,color:"#d0d0d0",detail:"18h 24m · 1h 36m left",due:"Ends Sunday"},{title:"Complete activity timeline",type:"Sprint",value:68,color:"#f2f2f2",detail:"7 of 10 tasks",due:"2 days left"},{title:"30-day consistency",type:"Long term",value:40,color:"#9a9a9a",detail:"12 of 30 days",due:"18 days left"},{title:"One deep session daily",type:"Daily",value:100,color:"#bdbdbd",detail:"2h 13m completed",due:"Complete"}];

export function BillingPage(){return <div><PageHeader eyebrow="Subscription" title="Billing" description="Plan, usage, invoices, and payment method." action={<button className="inline-flex min-h-11 items-center rounded-lg border border-white/[.08] px-4 text-sm text-[#cacaca]">Billing help</button>}/><Panel className="relative overflow-hidden border-[#f2f2f2]/25 p-6 sm:p-8"><div className="absolute right-0 top-0 h-full w-1/2 bg-[radial-gradient(circle_at_80%_20%,rgba(128,128,128,.18),transparent_60%)]"/><div className="relative grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end"><div><Tag color="violet"><Zap className="size-3"/> Premium</Tag><h2 className="mt-5 text-3xl font-semibold tracking-[-.05em]">Your full developer history, in motion.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-[#8c8c8c]">Advanced analytics, unlimited synchronized history, goals, share cards, and optional insights.</p><div className="mt-7 flex flex-wrap gap-6">{[["184h","synced focus"],["86%","focus score"],["14/∞","sessions this week"]].map(([v,l])=><div key={l}><p className="mono text-xl font-semibold">{v}</p><p className="mt-1 text-[10px] text-[#797979]">{l}</p></div>)}</div></div><div className="lg:text-right"><p className="mono text-4xl font-semibold">$8<span className="text-sm font-normal text-[#797979]"> / mo</span></p><p className="mt-2 text-xs text-[#7d7d7d]">Renews September 14</p><button className="mt-5 min-h-11 rounded-lg border border-white/10 px-4 text-sm text-[#cccccc]">Manage plan</button></div></div></Panel><div className="mt-3 grid gap-3 xl:grid-cols-[.75fr_1.25fr]"><Panel className="p-5"><h2 className="text-sm font-semibold">Payment method</h2><div className="mt-5 flex items-center gap-4 rounded-xl border border-white/[.07] bg-white/[.02] p-4"><div className="grid size-10 place-items-center rounded-lg bg-white/[.05]"><CreditCard className="size-4 text-[#d2d2d2]"/></div><div><p className="text-sm font-medium">•••• 4242</p><p className="mt-1 text-xs text-[#7a7a7a]">Expires 08/29</p></div><button className="ml-auto text-xs text-[#c2c2c2]">Update</button></div><h2 className="mt-7 text-sm font-semibold">Usage</h2><div className="mt-4"><Progress label="Cloud archive" value={36}/></div><p className="mt-3 text-[11px] text-[#797979]">1.8 GB of 5 GB included</p></Panel><Panel className="overflow-hidden"><div className="border-b border-white/[.07] p-5"><h2 className="text-sm font-semibold">Invoices</h2></div>{[["Aug 14, 2026","Sprintly Premium","$8.00","Paid"],["Jul 14, 2026","Sprintly Premium","$8.00","Paid"],["Jun 14, 2026","Sprintly Premium","$8.00","Paid"]].map(([d,p,a,s])=><div key={d} className="grid min-h-[68px] grid-cols-[1fr_auto] items-center gap-4 border-b border-white/[.06] px-5 last:border-0 sm:grid-cols-[140px_1fr_80px_80px_40px]"><span className="text-xs text-[#838383]">{d}</span><span className="hidden text-xs sm:block">{p}</span><span className="mono text-xs">{a}</span><Tag color="green">{s}</Tag><button aria-label={`Download invoice from ${d}`} className="grid size-9 place-items-center rounded-lg text-[#7a7a7a]"><Download className="size-4"/></button></div>)}</Panel></div></div>}
